...
$('.section').each(function(index){
  $(this).on('mousewheel DOMMouseScroll', function(event){
    ...
    if(wheelDelta<0){
      if(index<$('.section').length){ //0 1
        $('html').stop().animate({scrollTop:$(this).next().offset().top},500)
        console.log(index)
      }
    }else{
      if(index>0){ //1 2
        $('html').stop().animate({scrollTop:$(this).prev().offset().top},500)
      }
    }
  });
});
...
scrollEvent = setInterval(function(){
    mouseControll.animate({top:30},1000,'linear').animate({top:0},1000,'linear')
